...
import { Role, BookingStatus, RestaurantStatus } from "../../generated/prisma/enums.ts";
import { z } from "zod";
...
export const adminCreateUser = async (
  ...
  if (existing) return next(new AppError("Email already in use", 409, "USER_ALREADY_EXISTS"));
  ...
  return res.status(201).json(successResponse("User created", formatUser(user)));
};
...
export const adminUpdateUserStatus = async (
  ...
  const parsed = schema.safeParse(req.body);
  if (!parsed.success) return next(new AppError("Invalid status", 400, "VALIDATION_ERROR"));
...
export const adminUpdateRestaurantStatus = async (
  ...
  const id     = parseParamId(req.params.id);
  const schema = z.object({ status: z.enum(["active", "inactive", "pending", "suspended"]) });
  const parsed = schema.safeParse(req.body);
  if (!parsed.success) return next(new AppError("Invalid status", 400, "VALIDATION_ERROR"));

  const dbStatus = restaurantStatusMap[parsed.data.status] ?? RestaurantStatus.PENDING_APPROVAL;
  ...
  return res.json(successResponse("Restaurant status updated", formatRestaurant(restaurant)));
};
...
export const adminUpdateRestaurant = async (
  ...
  if (status)      updateData.status        = restaurantStatusMap[status] ?? RestaurantStatus.PENDING_APPROVAL;
...
export const adminListRequests = async (
  ...
  const [requests, total] = await Promise.all([
    prisma.restaurantRequest.findMany({ where, skip, take: limit, orderBy: { created_at: "desc" }, include: requestInclude }),
...
export const adminReviewRequest = async (
  ...
  const adminRecord = await prisma.admin.findUnique({ where: { user_id: req.user!.id } });
  if (!adminRecord) return next(new AppError("Admin record not found", 403, "FORBIDDEN"));

  const restaurantRequest = await prisma.restaurantRequest.findUnique({
    where: { id },
    include: requestInclude,
  });
  if (!restaurantRequest) return next(new AppError("Request not found", 404, "NOT_FOUND"));
  if (restaurantRequest.status !== "PENDING") {
    return next(new AppError("Request already reviewed", 400, "ALREADY_REVIEWED"));
  }
